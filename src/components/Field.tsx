...
const Field = forwardRef<HTMLInputElement|HTMLTextAreaElement, FieldProps>(function Field(
    {
    ...
    useEffect(() => {
        ...
        if (imprintModel && !Array.isArray(imprintModel) && imprintModel.value !== value)
        ...
        // string length check
            ...
            setError(
                checkReturnValidErrorMsg(minLength, `minimum ${minLength} characters`),
            );
        else if (isValStr && maxLength && value.length > checkReturnValidValue(maxLength))
            setError(
                checkReturnValidErrorMsg(maxLength, `maximum ${maxLength} characters`),
            );
        // pattern matching
    ...
    useEffect(() => {
        if (imprintModel && (max || min || maxLength || minLength || pattern || validate))
            throw new TypeError(
